...
func (u *programApprovalUsecase) GetCurriculumCommitteeApprovalByProgramUID(programUID, userUID *uuid.UUID, userApprovalLevel uint) (result *dto.GetApprovalCurriculumCommitteeDto, err error) {
	approval, committee, err := u.ProgramApprovalRepository.GetCurriculumCommitteeApprovalByProgramUID(*programUID, *userUID)
	if err != nil {
	...
	sort.SliceStable(committee, func(i, j int) bool {
		return committee[i].Email < committee[j].Email
	})
	...
	result = &dto.GetApprovalCurriculumCommitteeDto{
		...
		IsEditable:                  u.CheckIfApprovalEditable(uint(constant.CURRICULUM_COMMITEE_APPROVAL_LEVEL), userApprovalLevel),
