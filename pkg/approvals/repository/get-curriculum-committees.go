...
func (r *approvalRepository) GetCurriculumCommitteesApprovalByProgramUID(programUID uuid.UUID) (*migrateModels.ProgramApproval, []dto.GetApprovalCommitteesDto, error) {
	...
	if err := r.MainDbConn.Raw(submissionQueryWithCommittees, curriculumApproval.ID).Find(&currSubmissionQuery).Error; err != nil {
		log.Printf(constant.WHERE_AM_I_LOG, helpers.WhereAmI(), err.Error())
		return nil, committees, err
	}
