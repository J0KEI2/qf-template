...
func (r *approvalRepository) GetAcademicApprovalByProgramUID(programUID uuid.UUID) (*dto.GetApprovalAcademicDto, error) {
	...
	if err := r.MainDbConn.Model(&migrateModels.ProgramApproval{}).Where(&migrateModels.ProgramApproval{ProgramUID: programUID, ApprovalStatusLevel: constant.ACADEMIC_COUNCIL_APPROVAL_STATUS.EnumIndex()}).Take(&approvalData).Error; err != nil {
		log.Printf(constant.WHERE_AM_I_LOG, helpers.WhereAmI(), err.Error())
		return nil, err
	}

	// var submissions []dto.ApprovalSubmissionDto
	submissions := make([]dto.ApprovalSubmissionDto, 0)
	if err := r.MainDbConn.Raw(submissionQuery, approvalData.ID).Scan(&submissions).Error; err != nil {
		log.Printf(constant.WHERE_AM_I_LOG, helpers.WhereAmI(), err.Error())
		return nil, err
	}

	academicApproval := dto.GetApprovalAcademicDto{
		ID:                  approvalData.ID,
