package delivery

import (
	"net/http"
	"strconv"
	"strings"

	"github.com/gofiber/fiber/v2"
	helpers "github.com/zercle/gofiber-helpers"
	"github.com/zercle/kku-qf-services/pkg/helper"
)

func (h programHandler) GetCurriculumMapping() fiber.Handler {
	return func(c *fiber.Ctx) (err error) {
		responseForm := helpers.ResponseForm{}

		subPlanID, err := strconv.Atoi(c.Params("subPlanID", ""))
		curMapType := c.Params("curMapType", "")

		if err != nil {
			responseForm.Success = false
			responseForm.Errors = append(responseForm.Errors, helpers.ResponseError{
				Code:    http.StatusBadRequest,
				Title:   http.StatusText(http.StatusBadRequest),
				Message: err.Error(),
			})
			return c.Status(http.StatusBadRequest).JSON(responseForm)
		}

		paginationOptions := helper.ExtractPaginationOption(c)

		if strings.ToLower(curMapType) == "resp" {
			respModel, err := h.programUseCase.GetCurriculumMapping(paginationOptions, uint(subPlanID))
			if err != nil {
				return c.Status(http.StatusInternalServerError).JSON(responseForm)
			}

			responseForm.Result = respModel
		} else if strings.ToLower(curMapType) == "ksa" {
			respModel, err := h.programUseCase.GetCurriculumMappingKsa(paginationOptions, uint(subPlanID))
			if err != nil {
				return c.Status(http.StatusInternalServerError).JSON(responseForm)
			}

			responseForm.Result = respModel
		}

		responseForm.Success = true

		return c.JSON(responseForm)
	}
}
