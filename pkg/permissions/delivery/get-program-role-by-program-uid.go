...
func (h PermissionHandler) GetProgramRoleByProgramUID() fiber.Handler {
	return func(c *fiber.Ctx) (err error) {
		responseForm := helpers.ResponseForm{}

		programUID, err := uuid.Parse(c.Params("uid"))
		roleType := c.Query("role", "all")
		...
		respModel, err := h.permissionUsecase.GetProgramRolesByProgramUid(&programUID, &roleType, options)
		if err != nil {
			responseForm.Success = false
			responseForm.Errors = append(responseForm.Errors, helpers.ResponseError{
